...
void set_delta_constants()
{
  ...
  */
    ...
    delta_tower1_x = (-SIN_A * delta_radius) + tower_adj[0]; // front left tower + xa
    delta_tower1_y = (-COS_A * delta_radius) - tower_adj[0] ;
    delta_tower2_x = -(-SIN_B * delta_radius) + tower_adj[1]; // front right tower + xb
    delta_tower2_y = (-COS_B * delta_radius) + tower_adj[1]; //
    delta_tower3_x = tower_adj[2] ; // back middle tower + xc
    delta_tower3_y = -2 * (-COS_60 * delta_radius);
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      #if defined(PARKING_GCODE_1)
        SERIAL_ECHO("card.percent: ");
        SERIAL_ECHOLN(card.percentDone());
 //     if (card.percentDone()>60){
        SERIAL_ECHOLN("card.percent ok");
        enquecommand(PARKING_GCODE_1);
 //     #if defined(PARKING_GCODE_2)  
        enquecommand(PARKING_GCODE_2);
//      #endif    
 //     #if defined(PARKING_GCODE_3)  
        enquecommand(PARKING_GCODE_3);
 //     #endif 
 //       enquecommand("M300 S2000 P1000");
 //     }
      #endif
    ...
    case 30: //G30 Delta AutoCalibration
      ...
      if (code_seen('C'))
       ...
       /*       else  // убираем стремную калибровку длины диагонали из G30A
                {   
 ...
 /* Закомментировано для предотвращения испуга пользователя
    {
